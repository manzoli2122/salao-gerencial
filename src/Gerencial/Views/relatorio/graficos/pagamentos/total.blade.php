<script>  
  $(function () {
    ...
    var totalChartOptions = {
  	...
  	var ip = 0;
	@for ($i = 30; $i > 0; $i-- )
		label_diario_pagamento_total.push(["{{today()->subDays($i)->format('d/m')}} " ])    
		dados_diario_pagamento_total.push([ {{ Manzoli2122\Salao\Atendimento\Models\Pagamento::whereDate('created_at', [  today()->subDays($i)  ])->sum('valor') }}   ])
		ip = ip + 1;
	@endfor
